...
inline constexpr char kRobotWebControlPage[] = R"CONTROL(<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport"
    ...
    * {
    ...
    body {
      min-height: 100dvh;
      background: radial-gradient(circle at 45% -25%,#3a2d19 0,transparent
        42%),linear-gradient(180deg,#0e0b07,var(--bg));
    ...
    .eyes i {
      ...
      border-radius: 3px;
      background: var(--hi);
      box-shadow: 0 0 10px rgba(227,194,123,.28);
    }
    h1 {
      font-size: 18px;
      line-height: 1.1;
    ...
    .dot {
      ...
      box-shadow: 0 0 0 3px rgba(143,123,90,.12);
    }
    .online.ok {
      color: #a9d9bd;
      border-color: #315f48;
    ...
    .dashboard {
      display: grid;
      grid-template-columns: minmax(280px,.74fr) minmax(390px,.96fr) minmax(390px,.96fr);
      gap: var(--gap);
      align-items: start;
    ...
    .card {
      ...
      border: 1px solid var(--line);
      border-radius: 18px;
      background: linear-gradient(160deg,rgba(31,24,15,.96),rgba(17,14,9,.98));
      box-shadow: 0 14px 34px var(--shadow),inset 0 1px rgba(255,255,255,.025);
      padding: 17px;
      overflow: hidden;
    }
    .card:before {
    ...
    .control {
      ...
      border-radius: 16px;
      box-shadow: 0 6px 14px var(--shadow),inset 0 1px rgba(255,255,255,.04);
      cursor: pointer;
      touch-action: none;
    ...
    .vu i {
      ...
      width: 0;
      height: 100%;
      background: linear-gradient(90deg,var(--green) 0 68%,var(--yellow) 82%,var(--red));
    ...
    .action.on .icon {
      background: var(--brass);
      color: #130f08;
      box-shadow: 0 0 16px rgba(198,161,91,.2);
    }
    .threshold {
      margin-top: 11px;
      padding-top: 11px;
    ...
    .camera-body {
      display: grid;
      grid-template-columns: minmax(170px,1.15fr) minmax(140px,.7fr);
      gap: 12px;
      align-items: start;
    }
    .snapshot {
    ...
    .emotion.on {
      ...
      background: #2a2114;
      color: var(--hi);
      box-shadow: inset 0 0 0 1px rgba(227,194,123,.12);
    }
    .device-grid {
      display: grid;
      grid-template-columns: 1fr 1fr;
    ...
    .oled-page {
      ...
      overflow: hidden;
      color: #fff;
      font: 600 clamp(8px,1.7vw,11px)/1 ui-monospace,SFMono-Regular,Menlo,monospace;
    }
    .oled-cell {
      position: absolute;
      display: grid;
    ...
    .log-output {
      ...
      background: #070604;
      color: #d9c8a8;
      font: 10px/1.52 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;
      white-space: pre-wrap;
      overflow-wrap: anywhere;
      tab-size: 2;
    }
    ...
    @keyframes pulse {
      to {
        box-shadow: 0 0 0 5px rgba(216,101,91,.14),0 10px 32px #000;
      }
    }
    .toast {
      position: fixed;
...
<section class="card"><div class="section-title"><strong>Device</strong><span id="version">—</span></div><div class="device-grid"><div class="device-row"><span>Network</span><b id="ssid">—</b></div><div class="device-row"><span>Signal</span><b id="rssi">—</b></div><div class="device-row"><span>IP address</span><b id="ip">—</b></div><div class="device-row"><span>Internal RAM</span><b id="sram">—</b></div><div class="device-row"><span>PSRAM free</span><b id="psram">—</b></div><div class="device-row"><span>Camera health</span><b id="cameraHealth">—</b></div></div></section>
<section class="card"><div class="section-title"><strong>Robot status</strong><span id="uptime">—</span></div><div class="status"><div class="metric"><div class="label">Assistant</div><div class="value" id="state">—</div></div><div class="metric"><div class="label">Camera</div><div class="value" id="camera">—</div></div><div class="metric"><div class="label">Floor distance</div><div class="value" id="distance">—</div></div><div class="metric"><div class="label">Cliff sensor</div><div class="value" id="rangeState">Checking</div></div><div class="metric"><div class="label">Battery</div><div class="value" id="battery">Checking</div></div><div class="metric"><div class="label">Current · power</div><div class="value" id="power">—</div></div><div class="metric wide"><div class="label">Motion sensor</div><div class="value" id="motionSensor">Checking</div></div><div class="metric wide"><div class="label">Secondary OLED</div><div class="value health" id="oledState"><i class="dot"></i><span>Checking</span></div></div></div><div class="capacity-test"><div class="capacity-result"><b id="capacityResult">0.0 mAh</b><small id="capacityState">Capacity test not started</small><small id="socState">SoC waiting for INA219</small></div><div class="capacity-buttons"><button id="capacityStart">Start</button><button id="capacityStop">Stop</button><button id="capacityReset" class="reset">Reset</button></div></div><div class="threshold"><div class="threshold-head"><span>Cliff threshold</span><b id="cliffValue">150 mm</b></div><input id="cliffThreshold" type="range" min="50" max="500" step="5" value="150"></div><div class="segment-toggles" style="margin-top:11px;grid-template-columns:1fr 1fr"><label class="toggle"><input id="motionEmotions" type="checkbox" checked>MPU face reactions</label><label class="toggle"><input id="emotionMovement" type="checkbox">Emotion movement</label></div></section>
<section class="card"><div class="section-title"><strong>Audio &amp; lighting</strong></div><div class="audio-row"><span class="audio-name"><b>Speaker</b><small>Output volume</small></span><input id="speakerVolume" type="range" min="0" max="100" step="5" value="70"><span class="audio-value" id="speakerValue">70%</span></div><div class="audio-row"><span class="audio-name"><b>Microphone</b><small>Input gain</small></span><input id="microphoneGain" type="range" min="1" max="3" step="1" value="1"><span class="audio-value" id="microphoneValue">1×</span><div class="vu-wrap"><div class="vu"><i id="micLevel"></i></div><span class="clip" id="micClip">LIVE</span></div></div><div class="audio-row"><span class="audio-name"><b>Screen</b><small>Backlight</small></span><input id="screenBrightness" type="range" min="10" max="100" step="5" value="75"><span class="audio-value" id="screenValue">75%</span></div><div class="audio-row"><span class="audio-name"><b>Status light</b><small>Edison LED · GPIO48</small></span><input id="statusLightBrightness" type="range" min="0" max="100" step="5" value="100"><span class="audio-value" id="statusLightValue">100%</span></div></section>
<section class="card"><div class="section-title"><strong>Emotions</strong><span>5-second preview</span></div><div class="emotion-grid"><button class="emotion" data-emotion="neutral">Neutral</button><button class="emotion" data-emotion="happy">Happy</button><button class="emotion" data-emotion="bored">Bored</button><button class="emotion" data-emotion="laughing">Laughing</button><button class="emotion" data-emotion="funny">Funny</button><button class="emotion" data-emotion="sad">Sad</button><button class="emotion" data-emotion="angry">Angry</button><button class="emotion" data-emotion="crying">Crying</button><button class="emotion" data-emotion="loving">Loving</button><button class="emotion" data-emotion="embarrassed">Embarrassed</button><button class="emotion" data-emotion="surprised">Surprised</button><button class="emotion" data-emotion="shocked">Shocked</button><button class="emotion" data-emotion="thinking">Thinking</button><button class="emotion" data-emotion="winking">Winking</button><button class="emotion" data-emotion="cool">Cool</button><button class="emotion" data-emotion="relaxed">Relaxed</button><button class="emotion" data-emotion="delicious">Delicious</button><button class="emotion" data-emotion="kissy">Kissy</button><button class="emotion" data-emotion="confident">Confident</button><button class="emotion" data-emotion="sleepy">Sleepy</button><button class="emotion" data-emotion="silly">Silly</button><button class="emotion" data-emotion="confused">Confused</button><button class="emotion" data-emotion="suspicious">Suspicious</button><button class="emotion" data-emotion="shake">Shake</button></div></section>
</div>
<div class="column center-column">
<section class="card"><div class="section-title"><strong>Drive</strong><span id="durationLabel">250 ms</span></div><div class="dpad"><button class="control rotate-left-90" data-turn="-90" data-label="90°" aria-label="Rotate left 90 degrees" title="Rotate left 90°"><svg viewBox="0 0 24 24"><path d="M7.4 6.4 6 5 2 9l4 4 1.4-1.4L5.8 10H13a5 5 0 1 1-4.6 7H6.2a7 7 0 1 0 6.8-9H5.8z"/></svg></button><button class="control up" data-drive="forward" aria-label="Forward"><svg viewBox="0 0 24 24"><path d="M12 4 4 12h5v8h6v-8h5z"/></svg></button><button class="control rotate-right-90" data-turn="90" data-label="90°" aria-label="Rotate right 90 degrees" title="Rotate right 90°"><svg viewBox="0 0 24 24"><path d="M7.4 6.4 6 5 2 9l4 4 1.4-1.4L5.8 10H13a5 5 0 1 1-4.6 7H6.2a7 7 0 1 0 6.8-9H5.8z"/></svg></button><button class="control left" data-drive="left" aria-label="Left"><svg viewBox="0 0 24 24"><path d="M12 4 4 12h5v8h6v-8h5z"/></svg></button><button class="control stop" data-stop>STOP</button><button class="control right" data-drive="right" aria-label="Right"><svg viewBox="0 0 24 24"><path d="M12 4 4 12h5v8h6v-8h5z"/></svg></button><button class="control rotate-left-180" data-turn="-180" data-label="180°" aria-label="Rotate left 180 degrees" title="Rotate left 180°"><svg viewBox="0 0 24 24"><path d="M7.4 6.4 6 5 2 9l4 4 1.4-1.4L5.8 10H13a5 5 0 1 1-4.6 7H6.2a7 7 0 1 0 6.8-9H5.8z"/></svg></button><button class="control down" data-drive="backward" aria-label="Backward"><svg viewBox="0 0 24 24"><path d="M12 4 4 12h5v8h6v-8h5z"/></svg></button><button class="control rotate-right-180" data-turn="180" data-label="180°" aria-label="Rotate right 180 degrees" title="Rotate right 180°"><svg viewBox="0 0 24 24"><path d="M7.4 6.4 6 5 2 9l4 4 1.4-1.4L5.8 10H13a5 5 0 1 1-4.6 7H6.2a7 7 0 1 0 6.8-9H5.8z"/></svg></button></div><div class="range-row"><span>50 ms</span><input id="duration" type="range" min="50" max="5000" step="50" value="250"><span>5 sec</span></div><div class="range-row" style="margin-top:12px"><span>Speed</span><input id="motorSpeed" type="range" min="55" max="100" step="5" value="100"><span id="motorSpeedValue">100%</span></div><div class="motor-state"><span>Motion <b id="motion">Stopped</b></span><span>Queue <b id="queue">0</b></span><span>Time <b id="motorTime">0 ms</b></span></div><div class="progress"><i id="danceProgress"></i></div></section>
<section class="card"><div class="section-title"><strong>Displays</strong><span>Saved automatically</span></div><div class="display-actions"><button class="action" id="displayFlip" data-action="display_flip"><span class="icon">↕</span><span><b>Main display</b><small>Rotate 180°</small></span></button><button class="action" id="oledFlip" data-action="oled_flip"><span class="icon">↕</span><span><b>Secondary OLED</b><small>Rotate 180°</small></span></button></div><div class="threshold"><div class="threshold-head"><span>OLED contrast</span><b id="oledContrastValue">50%</b></div><input id="oledContrast" type="range" min="0" max="100" step="1" value="50"></div><div class="display-settings"><div class="oled-designer"><div class="oled-preview-head"><span>Layout preview</span><b id="oledPageCount">—</b></div><div id="oledPreview" class="oled-pages"><div class="oled-empty">Waiting for OLED configuration</div></div><div class="oled-preview-head"><span>Widgets · display order</span><span>Use arrows to reorder</span></div><div id="oledWidgets" class="oled-widget-list"></div></div></div></section>
<p class="hint">Local network only · tap a direction to run for the selected time · STOP cancels</p>
</div>
...
<section class="card log-card"><div class="log-head"><div class="log-state"><i></i><span id="logState">Live system log</span><span id="errorBadge" class="badge">0</span></div><div class="log-tools"><input id="logSearch" class="log-search" placeholder="Search logs"><select id="logFilter" class="log-select"><option value="all">All</option><option value="error">Errors</option><option value="warn">Warnings</option><option value="info">Info</option></select><button id="pauseLog" class="log-button">Pause</button><button id="downloadLog" class="log-button">Save</button><button id="clearLog" class="log-button">Clear</button></div></div><pre id="logOutput" class="log-output">Waiting for system logs…</pre><div class="log-foot"><span id="logSize">16 KB device buffer</span><label class="auto-scroll"><input id="autoScroll" type="checkbox" checked> Auto-scroll</label></div></section></div>
</div>
</main>
<button id="panicStop" class="panic-stop" data-stop>STOP MOTORS</button><div class="toast" id="toast"></div>
<script>
...
function oledModeOptions(type, selected) {
  ...
  return modes.map((name, index) =>
    '<option value="' + index + '" ' + (index === selected ? 'selected' : '') + '>' + name +
    '</option>'
  ).join('');
}
function oledPages(widgets) {
  const enabled = widgets.filter((w) => w.enabled), pages = [];
  let cursor = 0;
  while (cursor < enabled.length) {
    const left = enabled.length - cursor;
    if (left >= 3 && enabled.slice(cursor, cursor + 3).every((w) => w.size === 0)) {
      pages.push(
      ...
      cursor += 3;
    } else if (left >= 2 && enabled[cursor].size < 2 && enabled[cursor + 1].size < 2) {
      pages.push([{ w: enabled[cursor], x: 0, y: 0, width: 50, height: 100 }, {
        w: enabled[cursor + 1],
        x: 50,
        y: 0,
        width: 50,
      ...
      pages.push([{ w: enabled[cursor], x: 0, y: 0, width: 100, height: 100 }]);
      cursor++;
    }
  }
  return pages;
...
function renderOledConfig(j) {
  ...
  const signature = JSON.stringify([widgets, j.oled_brand, j.oled_distance_prefix]),
    list = $('#oledWidgets');
  const activeElement = document.activeElement;
  const editingOledCustomText =
    !!(activeElement && activeElement.matches && activeElement.matches('[data-oled-custom]'));
  if (signature !== oledWidgetSignature && !editingOledCustomText) {
    oledWidgetSignature = signature;
    list.innerHTML = widgets.map((w, index) => {
      const meta = oledMeta[w.type] || [w.type, ''];
      const custom = w.type === 'branding'
        ? '<label class="oled-custom"><span>Custom text</span><input class="text-input" data-oled-custom="brand" maxlength="20" value="' +
          escapeHtml(j.oled_brand || '') + '"></label>'
        : w.type === 'distance'
        ? '<label class="oled-custom"><span>Custom prefix</span><input class="text-input" data-oled-custom="prefix" maxlength="10" value="' +
          escapeHtml(j.oled_distance_prefix || '') + '"></label>'
        : '';
      return '<div class="oled-widget"><div class="oled-order"><button data-oled-up="' + index +
        '" ' + (index === 0 ? 'disabled' : '') +
        ' aria-label="Move up">↑</button><button data-oled-down="' + index + '" ' +
        (index === widgets.length - 1 ? 'disabled' : '') +
        ' aria-label="Move down">↓</button></div><label class="toggle"><input type="checkbox" data-oled-on="' +
        index + '" ' + (w.enabled ? 'checked' : '') + '></label><div class="oled-widget-name"><b>' +
        escapeHtml(meta[0]) + '</b><small>' + escapeHtml(meta[1]) +
        '</small></div><select data-oled-size="' + index + '"><option value="0" ' +
        (w.size === 0 ? 'selected' : '') + '>S · compact</option><option value="1" ' +
        (w.size === 1 ? 'selected' : '') + '>M · medium</option><option value="2" ' +
        (w.size === 2 ? 'selected' : '') + '>L · full</option></select><select data-oled-mode="' +
        index + '" ' + ((w.type === 'branding' || w.type === 'distance') ? 'disabled' : '') + '>' +
        oledModeOptions(w.type, w.mode) + '</select>' + custom + '</div>';
    }).join('');
    $$('[data-oled-on]').forEach((el) =>
      el.onchange = () =>
        action('oled_widget_on_' + el.dataset.oledOn, { value: el.checked ? 1 : 0 })
    );
    $$('[data-oled-size]').forEach((el) =>
      el.onchange = () => action('oled_widget_size_' + el.dataset.oledSize, { value: +el.value })
    );
    $$('[data-oled-mode]').forEach((el) =>
      el.onchange = () => action('oled_widget_mode_' + el.dataset.oledMode, { value: +el.value })
    );
    $$('[data-oled-up]').forEach((el) =>
      el.onclick = () => action('oled_widget_up_' + el.dataset.oledUp)
    );
    $$('[data-oled-down]').forEach((el) =>
      el.onclick = () => action('oled_widget_down_' + el.dataset.oledDown)
    );
    $$('[data-oled-custom]').forEach((el) => {
      el.onchange = () =>
        action(el.dataset.oledCustom === 'brand' ? 'oled_brand' : 'oled_prefix', {
          text: el.value,
        });
      el.onkeydown = (e) => {
  ...
  const pages = oledPages(widgets),
    ...
    pageCount = Number.isFinite(reported) && reported > 0 ? reported : pages.length;
    ...
    preview.innerHTML = pages.length
      ? pages.map((page, pageIndex) =>
        '<div class="oled-page">' + page.map((cell) =>
          '<div class="oled-cell" style="left:' + cell.x + '%;top:' + cell.y + '%;width:' +
          cell.width + '%;height:' + cell.height + '%">' +
          escapeHtml((oledMeta[cell.w.type] || [cell.w.type, '', cell.w.type])[2]) + '</div>'
...
function renderConversation(conversation) {
  ...
  if (signature !== conversationSignature) {
    ...
    const history = $('#conversationHistory'),
      pinned = history.scrollHeight - history.scrollTop - history.clientHeight < 30;
...
async function submitChat() {
  const input = $('#chatInput'), text = input.value, count = Array.from(text).length;
  if (chatSubmitting || !text.trim() || count > 80) return;
  chatSubmitting = true;
...
function renderStatus(j) {
  ...
  const idle = j.state === 'idle',
    ...
    chatActive = ['connecting', 'listening', 'speaking'].includes(j.state);
  ...
  $('#camera').textContent = !j.camera_available
    ...
    : (j.camera_flipped ? 'Flipped' : 'Normal');
  $('#distance').textContent = j.distance_valid ? j.distance_mm + ' mm' : '—';
  ...
  const batteryReady = !!j.battery_available && !!j.battery_valid,
    ...
    currentMa = Number.isFinite(signedCurrent) ? signedCurrent : Number(j.battery_current_ma),
    flow = j.battery_charging ? 'Charging' : j.battery_discharging ? 'Discharging' : 'Near zero';
  $('#battery').textContent = !j.battery_available
    ? 'Offline'
    : batteryReady
    ? flow + ' · ' + j.battery_percent + '% · ' + Number(j.battery_voltage_v).toFixed(2) + ' V'
    : 'Waiting';
  $('#power').textContent = batteryReady
    ? (currentMa >= 0 ? '+' : '') + Math.round(currentMa) + ' mA · ' +
      (Number(j.battery_power_mw) / 1000).toFixed(2) + ' W'
    : j.battery_available
    ? 'Invalid · CNVR ' + (j.battery_conversion_ready ? 'yes' : 'no') + ' · OVF ' +
  ...
  $('#capacityReset').disabled = !j.battery_available || (!capacityActive && capacityMah === 0);
  $('#capacityStart').textContent = capacityMah > 0 ? 'Resume' : 'Start';
  $('#capacityStart').classList.toggle('on', capacityMeasuring);
  const motionReady = !!j.motion_sensor_available && !!j.motion_sensor_valid;
  $('#motionSensor').textContent = !j.motion_sensor_available
    ? 'Offline'
    : motionReady
    ? (j.motion_gesture || 'steady') + ' · ' + Number(j.motion_roll_deg).toFixed(0) + '° / ' +
  ...
  if (document.activeElement !== $('#duration') && Number.isFinite(j.drive_duration_ms)) {
    $('#duration').value = j.drive_duration_ms;
    $('#durationLabel').textContent = fmtMs(j.drive_duration_ms);
  }
  $$('[data-turn]').forEach((b) => b.disabled = cliff || j.gyro_turn_pending || active);
  setRange($('#motorSpeed'), j.motor_speed, $('#motorSpeedValue'), '%');
  $('#safety').classList.toggle('show', cliff);
  $$('[data-drive]').forEach((b) => b.disabled = cliff && b.dataset.drive !== 'backward');
  const oled = $('#oledState');
  oled.className = 'value health ' + (j.oled_available ? 'good' : 'bad');
  oled.querySelector('span').textContent = j.oled_available ? 'Ready' : 'Offline';
  setRange($('#cliffThreshold'), j.cliff_edge_mm, $('#cliffValue'), ' mm');
  setRange($('#speakerVolume'), j.speaker_volume, $('#speakerValue'), '%');
  setRange($('#microphoneGain'), j.microphone_gain, $('#microphoneValue'), '×');
  setRange($('#screenBrightness'), j.screen_brightness, $('#screenValue'), '%');
  setRange($('#statusLightBrightness'), j.status_light_brightness, $('#statusLightValue'), '%');
  if (Number.isFinite(j.oled_contrast)) {
    const contrastPercent = Math.round(Math.max(0, Math.min(255, j.oled_contrast)) * 100 / 255);
    setRange($('#oledContrast'), contrastPercent, $('#oledContrastValue'), '%');
  }
  $('#micLevel').style.width = Math.max(0, Math.min(100, j.microphone_level || 0)) + '%';
  ...
  $('#lightsAction').classList.toggle('on', (j.status_light_brightness || 0) > 0);
  $$('[data-emotion]').forEach((b) =>
    b.classList.toggle('on', b.dataset.emotion === (j.emotion || 'neutral'))
  );
  ...
  $('#danceState').textContent = m.sequence_active ? (done + '/' + total + ' steps') : 'Ready';
  ...
  $('#cameraHealth').textContent = j.camera_available ? 'Ready' : 'Offline';
  return active;
}
...
oledContrast.onchange = () =>
  action('oled_contrast', { value: Math.round(+oledContrast.value * 255 / 100) });
[
...
$$('[data-drive]').forEach((btn) => {
  btn.onclick = () => {
    if (!btn.disabled) action(btn.dataset.drive, { duration_ms: +duration.value });
  };
  btn.oncontextmenu = (e) => e.preventDefault();
});
$$('[data-turn]').forEach((btn) => {
  btn.onclick = () => {
    if (!btn.disabled) action('turn_relative', { value: +btn.dataset.turn });
  };
  btn.oncontextmenu = (e) => e.preventDefault();
});
...
$$('[data-action]').forEach((btn) =>
  btn.onclick = () => {
    ...
    if (
      name === 'wifi_config' && !confirm('Open Wi-Fi setup mode? This page will disconnect.')
    ) return;
    if (
      name === 'reboot' &&
      !confirm('Reboot the robot now? Motors will stop and this page will disconnect briefly.')
    ) return;
    action(name);
  }
);
$$('[data-emotion]').forEach((btn) =>
  btn.onclick = () => action('emotion', { text: btn.dataset.emotion, duration_ms: 5000 })
);
...
async function captureSnapshot(silent = false) {
  ...
  const button = $('#takeSnapshot'), meta = $('#snapshotMeta');
  button.disabled = true;
  if (!silent) {
  ...
  try {
    const r = await fetch('/api/camera/snapshot?ts=' + Date.now(), { cache: 'no-store' });
    if (!r.ok) {
...
function hasLogSelection() {
  ...
  return logOutput.contains(sel.anchorNode) || logOutput.contains(sel.focusNode);
}
function renderLogs(force = false) {
  ...
  const q = $('#logSearch').value.toLowerCase(),
    ...
    visible = lines.filter((line) =>
      (filter === 'all' || logLevel(line) === filter) && (!q || line.toLowerCase().includes(q))
    ),
    pinned = logOutput.scrollHeight - logOutput.scrollTop - logOutput.clientHeight < 30;
  logOutput.textContent = visible.join('\n') || 'No matching logs';
  const errors = lines.filter((l) => logLevel(l) === 'error').length, badge = $('#errorBadge');
...
$('#downloadLog').onclick = () => {
  const a = document.createElement('a'), blob = new Blob([allLogs], { type: 'text/plain' });
  a.href = URL.createObjectURL(blob);
  a.download = 'xiaozhi-' + new Date().toISOString().replaceAll(':', '-') + '.log';
  a.click();
