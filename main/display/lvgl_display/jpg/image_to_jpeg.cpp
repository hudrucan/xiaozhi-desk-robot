...
static bool encode_with_esp_new_jpeg(const uint8_t* src, uint16_t width, uint16_t height,
...
bool image_to_jpeg(uint8_t* src, size_t src_len, uint16_t width, uint16_t height, v4l2_pix_fmt_t format,
    ...
    return encode_with_esp_new_jpeg(src, width, height, format, quality, out, out_len, NULL, NULL);
}
...
#endif // CONFIG_XIAOZHI_CAMERA_ALLOW_JPEG_INPUT
    return encode_with_esp_new_jpeg(src, width, height, format, quality, NULL, NULL, cb, arg);
}
